...
import { Star, ShieldCheck, Gem, Award } from 'lucide-react';

const metrics = [
  { icon: Gem, value: '10,000+', label: 'Bespoke Pieces Created' },
  { icon: Star, value: '4.9/5', label: 'Average Client Rating' },
  { icon: ShieldCheck, value: '100%', label: 'Ethically Sourced Gems' },
  { icon: Award, value: 'Award-Winning', label: 'AI Design Engine' },
];

const brands = ['VOGUE', "HARPER'S BAZAAR", 'FORBES', 'GQ', 'ELLE'];

export function SocialProof() {
  return (
    <section className="py-24 bg-bg relative border-y border-black/5 dark:border-white/5 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
          <p className="text-sm font-medium text-text-secondary uppercase tracking-widest mb-8">
            Trusted by Connoisseurs & Recognized By
          </p>
          ...
          <div className="flex overflow-hidden relative w-full [mask-image:linear-gradient(to_right,transparent,black_10%,black_90%,transparent)]">
            ...
            >
              {[...brands, ...brands, ...brands].map((brand, i) => (
                <span
                  key={i}
                  className="text-2xl md:text-3xl font-serif text-text-primary/40 dark:text-text-primary/30 font-bold tracking-wider hover:text-text-primary/70 transition-colors"
                >
                  {brand}
                </span>
              ))}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 md:gap-12 mt-20">
          {metrics.map((metric, index) => (
            <motion.div
              ...
              className="flex flex-col items-center text-center p-6 rounded-3xl bg-card/30 backdrop-blur-sm border border-black/5 dark:border-white/5 hover:bg-card/60 transition-colors"
            >
